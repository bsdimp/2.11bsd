...
else	error("elseif out of place", 0,0,EXECERR);
}
...
else	error("else out of place", 0,0,EXECERR);
}
...
if(ctlstack->ctltype == CTLIF)
	...
	error("endif out of place", 0,0,EXECERR);
}
...
if(++ctlstack >= lastctl)
	error("nesting too deep",0,0,FATAL);
...
{
if( ctlstack-- < ctls )
	error("control stack empty",0,0,FATAL);
--blklevel;
}
...
if(lp->tag != TPRIM)
	{
	error("assignment to a non-variable",0,0,ERR);
	frexpr(lp);
	frexpr(rp);
	}
	...
	{
	if(parstate >= INEXEC)
		error("statement function amid executables",0,0,ERR);
	else
		mkstfunct(lp, rp);
	}
else
...
if(np->vclass == CLUNKNOWN)
	...
	{
	error("redeclaration of statement function", np, 0, DCLERR);
	return;
	}
...
for( ; args ; args = args->nextp)
	if( (p = args->datap)->tag!=TPRIM ||
		...
		error("non-variable argument in statement function definition",0,0,ERR);
	else
		{
...
if(p)
	...
	if( ! ISCONST(p) )
		{
		error("pause/stop argument must be constant", 0,0,EXECERR);
		frexpr(p);
		p = mkstrcon(0, 0);
		}
		...
		{
		error("pause/stop argument must be integer or string",
			0,0,EXECERR);
		p = mkstrcon(0, 0);
		}
	}
...
if(np->vdovar)
	{
	error("nested loops with variable %s", varstr(VL,np->varname),0,ERR1);
	ctlstack->donamep = NULL;
	return;
	}
...
if( ! ONEOF(dovarp->vtype, MSKINT|MSKREAL) )
	{
	error("bad type on do variable",0,0,ERR);
	return;
	}
ctlstack->donamep = np;
...
for(i=0 , cp = spec->nextp ; cp!=NULL && i<3 ; cp = cp->nextp)
	...
	if( ! ONEOF(p->vtype, MSKINT|MSKREAL) )
		{
		error("bad type on DO parameter",0,0,ERR);
		return;
		}
	}
...
switch(i)
	...
	case 0:
	case 1:
		error("too few DO parameters",0,0,ERR);
		return;

	default:
		error("too many DO parameters",0,0,ERR);
		return;
...
if( ISCONST(DOINCR) )
	...
	if( (incsign = conssgn(ctlstack->dostep)) == 0)
		error("zero DO increment",0,0,ERR);
...
if( ISCONST(ctlstack->domax) && ISCONST(DOINIT) && ctlstack->dostepsign!=VARSTEP)
	...
	if( onetripflag )
		...
		if(incsign != conssgn(q))
			{
			error("DO range never executed",0,0,WARN);
			putgoto(ctlstack->endlabel);
			}
		frexpr(q);
		}
...
if( ! ONEOF(p->vtype, MSKINT|MSKADDR) )
	error("noninteger assign variable",0,0,ERR);
else
...
if( ! ONEOF(expr->vtype, MSKINT|MSKREAL) )
	{
	error("invalid type of arithmetic if expression",0,0,ERR);
	frexpr(expr);
	}
else
	{
...
{
if(procclass != CLPROC)
	error("RETURN statement in main or block data",0,0,WARN);
if(p && (proctype!=TYSUBR || procclass!=CLPROC) )
	{
	error("alternate return in nonsubroutine",0,0,ERR);
	p = 0;
	}
...
if( ! ISINT(p->vtype) )
	error("assigned goto variable must be integer",0,0,ERR);
else
	putbranch(p);
}
