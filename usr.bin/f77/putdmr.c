...
if( (k = (p = fixtype(p))->vtype) != TYLOGICAL)
	{
	if(k != TYERROR)
		error("non-logical expression in IF statement",0,0,ERR);
	frexpr(p);
	}
else
	{
...
type = p->vtype;
if(p->tag != TADDR)
	error("invalid goto label",0,0,FATAL);
putaddr(p, YES);
if(type != TYINT)
	p2op2(P2LTOI, P2INT);
...
if(! ISINT(index->vtype) )
	{
	error("computed goto index must be integer", NULL,0,EXECERR);
	return;
	}
...
switch(p->tag)
	...
	case TEXPR:
		...
		putopp:
			...
			default:
				error("putx: invalid opcode %d", opc,0,FATAL1);
			}
		break;
	...
	default:
		error("putx: impossible tag %d", p->tag,0,FATAL1);
	}
}
...
switch(p->opcode)	/* check for special cases and rewrite */
	...
	case OPCONV:
		...
		switch(ptype = p->vtype)
			...
			default:
			badconv:
				error("putop: impossible conversion",0,0,FATAL);
			}
		putx(lp);
...
if( (k = ops2[p->opcode]) <= 0)
	error("putop: invalid opcode %d", p->opcode,0,FATAL1);
putx(p->leftp);
if(p->rightp)
	putx(p->rightp);
...
switch(p->tag)
	...
	default:
		error("putcx1: bad tag %d", p->tag,0,FATAL1);
	}
...
switch(opcode)
	...
	default:
		error("putcx1 of invalid opcode %d", opcode,0,FATAL1);
	}
	...
	case TEXPR:
		...
		switch(p->opcode)
			...
			case OPCONV:
				...
				break;
			default:
				error("putch1: invalid opcode %d", p->opcode,0,FATAL1);
			}
		return(t);

	default:
		error("putch1: bad tag %d", p->tag,0,FATAL1);
	}
/* NOTREACHED */
}

...
switch(p->vstg)
	...
	default:
		error("putaddr: invalid vstg %d", p->vstg,0,FATAL1);
	}
frexpr(p);
}
...
if(type == TYCHAR)
	...
	if( ISICON(p->vleng) )
		...
		error("adjustable character function",0,0,ERR);
		return(NULL);
		}
	}
...
#if SZINT < SZLONG
	/*
	 * Attempt to reduce the size of an index or
	 * subscript type in an expression.
	 * It is unclear whether this works.
	 * wfj note 10/80
	 */
	if(shortsubs)
		offp = shorten(offp);
