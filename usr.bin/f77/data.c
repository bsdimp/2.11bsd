...
if( ! ISCONST(valp) )
	{
	error("nonconstant initializer",0,0,ERR);
	goto ret;
	}
...
for(i = 0 ; i < nrep ; ++i)
	...
	if(p == NULL)
		{
		error("too many initializers",0,0,ERR);
		toomanyinit = YES;
		goto ret;
		}
...
while(curdtp)
	...
	if(p->argsp==NULL && np->vdim!=NULL)
		...
		if( (neltp = np->vdim->nelt) && ISCONST(neltp))
			...
			error("attempt to initialize adjustable array",0,0,ERR);
		}
	else
		q = mklhs( cpexpr(pp) );
	if(skip)
...
doerr:
		error("nonconstant implied DO parameter",0,0,ERR);
		frexpr(q);
		curdtp = curdtp->nextp;
...
if(type!=TYCHAR && valtype==TYCHAR)
	{
	if(! ftn66flag)
		error("noncharacter type initialized with string",0,0,WARN);
	...
	{
	error("incompatible types in initialization",0,0,ERR);
	return;
	}
if(type != TYCHAR)
...
switch(type)
	...
	default:
		error("setdata: impossible type %d", type,0,FATAL1);
	}
