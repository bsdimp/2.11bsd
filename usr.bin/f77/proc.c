...
if(parstate != OUTSIDE)
	{
	error("missing end statement", 0,0,EXECERR);
	endproc();
	}
...
if(parstate < INDATA)
	enddcl();
if(ctlstack >= ctls)
	error("DO loop or BLOCK IF not closed",0,0,ERR);
for(lp = labeltab ; lp < labtabend ; ++lp)
	if(lp->stateno!=0 && lp->labdefined==NO)
		error("missing statement number %s",convic(lp->stateno),0,ERR1);
...
if(p==NULL || p->extinit || ! ONEOF(p->extstg, M(STGUNKNOWN)|M(STGEXT)) )
	{
	if(p == 0)
		error("invalid entry name", v, 0, DCLERR);
	else	error("external name already used", v, 0, DCLERR);
	return(0);
	}
v->vstg = STGAUTO;
...
switch(t)
	...
	default:
		error("retval: impossible type %d", t,0,FATAL1);
	}
goret(t);
}
...
if(proctype == TYCHAR)
	...
	error("character entry of noncharacter function",0,0,ERR);
...
for(p = hashtab ; p<lasthash ; ++p)
	...
	if( (q->vclass==CLUNKNOWN && q->vstg!=STGARG) ||
		...
		error("local variable %s never used", varstr(VL,q->varname),0,WARN1);
	...
	if(q->vclass==CLVAR && q->vstg!=STGARG)
		...
		if(q->vdim && !ISICON(q->vdim->nelt) )
			error("adjustable dimension on non-argument", q, 0, DCLERR);
		if(q->vtype==TYCHAR && (q->vleng==NULL || !ISICON(q->vleng)))
			error("adjustable leng on nonargument", q, 0, DCLERR);
		}
	}
	...
	if(p->extstg==STGCOMMON)
		...
		for(q = p->extp ; q ; q = q->nextp)
			...
			if(p->extleng % typealign[type] != 0)
				{
				error("common alignment", v, 0, DCLERR);
...
for(p = extsymtab ; p < nextext ; ++p)
	if(p->extstg == STGCOMMON)
		...
		if(p->maxleng!=0 && p->extleng!=0 && p->maxleng!=p->extleng &&
			...
			error("incompatible lengths for common block %s",
				nounder(XL, p->extname),0,WARN1);
...
if(type==TYUNKNOWN || type==TYERROR)
	error("mktmpn: invalid type %d", type,0,FATAL1);
...
if(p->extstg == STGUNKNOWN)
	...
	{
	error("%s cannot be a common block name", s,0,ERR1);
	return(0);
	}
...
if(v->vstg != STGUNKNOWN)
	error("incompatible common declaration", v, 0, DCLERR);
else
	{
	v->vstg = STGCOMMON;
...
if(type==TYSUBR && v->vtype!=TYUNKNOWN && v->vstg==STGARG)
	...
	if(v->vstg == STGUNKNOWN)
		...
		error("incompatible storage declarations", v, 0, DCLERR);
	}
...
switch(type)
	...
	case TYLONG:
		...
		break;
	default:
		error("lengtype: invalid type %d", type,0,FATAL1);
	}

if(length != 0)
	error("incompatible type-length combination",0,0,ERR);
...
if(v->vstg == STGUNKNOWN)
	...
	error("incompatible use of intrinsic function", v, 0, DCLERR);
if(v->vclass==CLUNKNOWN)
	v->vclass = CLPROC;
if(v->vprocclass == PUNKNOWN)
	v->vprocclass = PINTRINSIC;
else if(v->vprocclass != PINTRINSIC)
	error("invalid intrinsic declaration", v, 0, DCLERR);
if(k = intrfunct(v->varname))
	v->vardesc.varno = k;
else
	error("unknown intrinsic function", v, 0, DCLERR);
}
...
if(v->vclass == CLUNKNOWN)
	...
	error("invalid external declaration", v, 0, DCLERR);
...
if(v->vclass == CLUNKNOWN)
	...
	{
	error("only variables may be arrays", v, 0, DCLERR);
	return;
	}
...
for(i=0 ; i<nd ; ++i)
	...
	if( (q = dims[i].ub) == NULL)
		...
		if(i == nd-1)
			...
			error("only last bound may be asterisk",0,0,ERR);
