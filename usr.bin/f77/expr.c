...
if( ISCONST(realp) && ISNUMERIC(rtype) && ISCONST(imagp) && ISNUMERIC(itype) )
	...
	{
	error("invalid complex constant",0,0,ERR);
	p = errnode();
	}
...
if(t==TYUNKNOWN || t==TYERROR)
	error("mkconv of impossible type %d", t,0,FATAL1);
pt = p->vtype;
if(t == pt)
	return(p);
...
switch(tag)
	...
	default:
		error("cpexpr: impossible tag %d", tag,0,FATAL1);
	}
...
switch(p->tag)
	...
	default:
		error("frexpr: impossible tag %d", p->tag,0,FATAL1);
	}
...
switch(p->tag)
	...
	default:
		error("fixtype: impossible tag %d", p->tag,0,FATAL1);
...
if(p->tag == TERROR)
	...
	error("fixexpr: invalid tag %d", p->tag,0,FATAL1);
opcode = p->opcode;
lp = p->leftp = fixtype(p->leftp);
ltype = lp->vtype;
if(opcode==OPASSIGN && lp->tag!=TADDR)
	{
	error("left side of assignment must be variable",0,0,ERR);
	frexpr(p);
	return( errnode() );
	}
...
switch(p->tag)
	...
	default:
		error("shorten: invalid tag %d", p->tag,0,FATAL1);
	}
...
if(class == CLUNKNOWN)
	...
	if(np->vstg == STGUNKNOWN)
		...
		if(np->vtype!=TYCHAR && !ftn66flag)
		    error("Dummy procedure not declared EXTERNAL. Code may be wrong.",0,0,WARN);
		np->vprocclass = PEXTERNAL;
		}
	}

if(class != CLPROC)
	error("invalid class code for function", class,0,FATAL1);
if(p->fcharp || p->lcharp)
	{
	error("no substring of function call",0,0,ERR);
	goto err;
	}
impldcl(np);
...
switch(np->vprocclass)
	...
	case PTHISPROC:
		error("recursive call",0,0,WARN);
		for(ep = entries ; ep ; ep = ep->nextp)
			if(ep->enamep == np)
				break;
		if(ep == NULL)
			error("mkfunct: impossible recursion",0,0,FATAL);
		ap = builtin(np->vtype, varstr(XL, ep->entryname->extname) );
		goto call;

	default:
		error("mkfunct: impossible vprocclass %d", np->vprocclass,0,FATAL1);
	}
free(p);
return(q);
	...
	if(q->vtype==ap->vtype && q->vtype!=TYCHAR
	   && (ap->tag==TCONST || ap->tag==TADDR) )
		...
		if( (rp->rpltag = rp->rplxp->tag) == TERROR)
			error("disagreement of argument types in statement function call",0,0,ERR);
		}
	rp->nextp = tlist;
	tlist = rp;
...
if(actuals!=NULL || formals!=NULL)
	error("statement function definition and argument list differ",0,0,ERR);
...
if(p->fcharp || p->lcharp)
	{
	if(np->vtype != TYCHAR)
		error("substring of noncharacter %s", varstr(VL,np->varname),0,ERR1);
	else	{
		if(p->lcharp == NULL)
...
if(p->argsp)
	for(cp = p->argsp->listp ; cp ; cp = cp->nextp)
		...
		if(n > 7)
			{
			error("more than 7 subscripts",0,0,ERR);
			break;
			}
		}
...
dimp = np->vdim;
if(n>0 && dimp==NULL)
	error("subscripts on scalar variable",0,0,ERR);
else if(dimp && dimp->ndim!=n)
	error("wrong number of subscripts on %s", varstr(VL, np->varname),0,ERR1);
else if(n > 0)
	{
	prod = sub[--n];
	while( --n >= 0)
...
badsub:
	frexpr(p);
	error("subscript on variable %s out of range", varstr(VL,np->varname),0,ERR1);
	return ( ICON(0) );
}
...
error("mkaddr: impossible storage tag %d", p->vstg,0,FATAL1);
/* NOTREACHED */
}
...
if(v->vclass == CLPARAM)
	...
	if(args || lstr || rstr)
		{
		error("no qualifiers on parameter name", varstr(VL,v->varname),0,ERR1);
		frexpr(args);
		frexpr(lstr);
		frexpr(rstr);
		frexpr(v);
...
if(v->vclass == CLUNKNOWN)
	...
	{
	error("used as variable", v, 0, DCLERR);
	return;
	}
...
if(p->vtype == TYUNKNOWN)
	...
	if(type == TYUNKNOWN)
		...
		if(p->vclass == CLPROC)
			return;
		error("attempt to use undefined variable", p, 0, DCLERR);
		type = TYERROR;
		leng = 1;
		}
	settype(p, type, leng);
...
switch(opcode)
	...
	case OPMOD:
		if( ICONEQ(rp, 0) )
			{
			error("attempted division by zero",0,0,ERR);
			rp = ICON(1);
			break;
			}
		if(opcode == OPMOD)
	...
	default:
		error("mkexpr: impossible opcode %d", opcode,0,FATAL1);
	}
...
switch(op)
	...
	case OPMOD:
		...
		error("nonarithmetic operand of arithmetic operator",0,0,ERR);goto err1;

	case OPNEG:
		if( ISNUMERIC(lt) )
			return(lt);
		error("nonarithmetic operand of negation",0,0,ERR);goto err1;

	case OPNOT:
		if(lt == TYLOGICAL)
			return(TYLOGICAL);
		error("NOT of nonlogical",0,0,ERR);goto err1;
	...
	case OPNEQV:
		if(lt==TYLOGICAL && rt==TYLOGICAL)
			return(TYLOGICAL);
		error("nonlogical operand of logical operator",0,0,ERR);goto err1;
	...
	case OPNE:
		if(lt==TYCHAR || rt==TYCHAR || lt==TYLOGICAL || rt==TYLOGICAL)
			{
			if(lt != rt)
				{error("illegal comparison",0,0,ERR);goto err1;}
			}

		else if( ISCOMPLEX(lt) || ISCOMPLEX(rt) )
			{
			if(op!=OPEQ && op!=OPNE)
				{error("order comparison of complex data",0,0,ERR);goto err1;}
			}

		else if( ! ISNUMERIC(lt) || ! ISNUMERIC(rt) )
			{error("comparison of nonarithmetic data",0,0,ERR);goto err1;}
		return(TYLOGICAL);

	case OPCONCAT:
		if(lt==TYCHAR && rt==TYCHAR)
			return(TYCHAR);
		error("concatenation of nonchar data",0,0,ERR);goto err1;
...
/* debug error("impossible conversion.  possible compiler bug",0,0,FATAL); */
				error("impossible conversion",0,0,ERR);goto err1;
				}
		return(lt);
	...
	default:
		error("cktype: impossible opcode %d", op,0,FATAL1);
	}
err1:	return(TYERROR);
}

...
if(rp == 0)
	switch(opcode)
		...
		default:
			error("fold: invalid unary operator %d", opcode,0,FATAL1);
		}
...
switch(type = ap->vtype)	/* pow = 1 */ 
	...
	case TYDREAL:
		...
		break;
	default:
		error("conspower: invalid type %d", type,0,FATAL1);
	}
...
if(n < 0)
	...
	if( ISINT(type) )
		{
		error("integer ** negative power ",0,0,ERR);
		return;
		}
	n = - n;
...
switch(opcode)
	...
	case OPMOD:
		if( ISINT(type) )
			...
			error("inline mod of noninteger",0,0,FATAL);
...
{
if( ! ISCONST(p) )
	error("sgn(nonconstant)" ,0,0,FATAL);
...
if( abr <= abi )
	{
	if(abi == 0)
		error("complex division by zero",0,0,FATAL);
